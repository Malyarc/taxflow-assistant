...
export function computeTaxReturnPure(inputs: TaxReturnInputs): ComputedTaxReturn {
  ...
  const totalInvestmentIncomeForNiit = Math.max(
    ...
    investmentIncomeFromAdj +
      ...
      // K-1 non-qualified, i.e. 6a − 6b) so NII stays consistent with the
      // dividends already in AGI (T1.0d #12 — 6b is a subset of 6a).
      form1099Summary.interestIncome +
      form1099Summary.ordinaryDividends +
      qualifiedDividends +
      k1InterestIncome +
  ...
  const amtCreditCarryforwardIn = Math.max(0, sumByType("amt_credit_carryforward")); // T1.0d #13 floored
