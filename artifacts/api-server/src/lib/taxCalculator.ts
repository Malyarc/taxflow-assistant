 ...
 */
function aptcRepaymentCap(taxYear: TaxYear, filingStatus: string, fplFraction: number): number | null {
  // NULL = no limitation (full repayment) — never Infinity (engine totality).
  if (taxYear >= 2026) return null; // OBBBA — limitation repealed
  if (!(fplFraction < 4.0)) return null; // ≥400% FPL — no cap (NaN-safe)
...
export function calculatePremiumTaxCredit(params: {
  ...
  if (params.filingStatus === "married_filing_separately") {
    ...
    return {
      ...
      computedPtc: 0,
      advanceAptc,
      // FC-10 — MFS is PTC-ineligible (§36B(c)(1)(C)) but the 8962
      // instructions apply Table 5 to each MFS spouse separately; a null
      // mfsCap (TY2026+/≥400%) means full repayment.
      repaymentCap: mfsCap,
      netPtc: mfsCap != null ? Math.max(-advanceAptc, -mfsCap) : -advanceAptc,
      eligible: false,
    };
  }
  ...
  if (params.annualPremium <= 0 || params.annualSlcsp <= 0 || params.householdSize <= 0) {
    return {
      ...
      // null = "no limitation applied" (any advance is repaid in full).
      repaymentCap: null,
  ...
  if (netPtc < 0) {
    // FC-10 columns (single half-caps vs "all other statuses") + the TY2026
    // OBBBA repeal + the ≥400% no-cap all live in aptcRepaymentCap (null = no
    // limitation — full repayment).
