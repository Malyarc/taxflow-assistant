...
header("E2 — live-app channel: lookback vs gainClass=\"section1250\" transactions");
{
  ...
  // Tax (RE-DERIVED post-merge under the T1.0l Schedule D Tax Worksheet
  // interleave — the flat-25% derivation this test originally carried is the
  // pre-worksheet world): L1 165,400; L9 50,000; L11/L12 §1250 20,000;
  // L13 ANCG 30,000; L14 = 135,400; L17 ord = 115,400;
  // L21 = max(115,400, min(135,400, 191,950)) = 135,400 → the §1250 pool is
  // FULLY absorbed at ordinary rates (L38 reduction = (50,000+135,400)
  // −165,400 = 20,000 → 25% bucket $0). 0% zone none (L14 > 47,025).
  //   ordinary tax(135,400) = 1,160 + 4,266 + 11,742.50 + 24%×34,875
  //     = 25,538.50; ANCG 30,000 @15% = 4,500 → federal = $30,038.50.
  const r = computeTaxReturnPure({
    client: { filingStatus: "single", state: "FL", taxYear: 2024 },
    w2s: [{ taxYear: 2024, wagesBox1: 100000, stateCode: "FL" }],
    form1099s: [],
  ...
  check("E2 federal tax = $30,038.50 (worksheet interleave)", r.federalTaxLiability, 30038.5, 0.5);
  // No-lookback control (worksheet): L9 80,000; §1250 50,000; ANCG 30,000;
  // L14 = 135,400; L17 = 85,400; L21 = max(85,400, 135,400) = 135,400 —
  // IDENTICAL ordinary base; L38 reduction = (80,000+135,400)−165,400 =
  // 50,000 → 25% bucket $0 again. federal = 25,538.50 + 4,500 = $30,038.50.
  // §1231(c) recharacterization is TAX-NEUTRAL when the §1250 pool sits
  // below the 24%-bracket top (both characters land at ordinary rates) —
  // the mechanics (pool erosion, CF consumption) still matter above it.
  const ctrl = computeTaxReturnPure({
    client: { filingStatus: "single", state: "FL", taxYear: 2024 },
    w2s: [{ taxYear: 2024, wagesBox1: 100000, stateCode: "FL" }],
    form1099s: [],
  ...
  check("E2 control federal tax = $30,038.50 (== lookback leg below the 24% top)", ctrl.federalTaxLiability, 30038.5, 0.5);
}
