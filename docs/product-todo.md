...
- [~] `[ENH]` **State "modifications" layer** (PREP-B1) — **PER-LINE NON-RESIDENT SOURCING SHIPPED 2026-06-05h.** `calculateMultiStateTax` non-resident branch uses the tax-ratio method (a) for **{CA, NY, CT, NJ, MN, GA, NC, OH}** (CT/NJ/MN added + method-(a)-verified against their NR forms 2026-06-06j/k; **GA/NC/OH added + method-(a)-verified 2026-06-08** — GA Form 500 Sch 3, NC D-400 Sch PN, OH IT NRC nonresident credit) (as-if-resident on total income × source-fraction; fixes the cross-border-commuter under-tax + the std-ded-over-deduction for flat-tax NR states), a per-income-type NR source base (new `perStateNonResidentOtherSourced` for NR business/rental/real-property gains), and 4 U.S.C. §114 enforced by design (intangibles + retirement never auto-sourced). 35 hand-calc'd tests; adversarially verified. **MD deliberately EXCLUDED + test-guarded** (Form 505NR is method b + a 2.25% special nonresident tax the engine doesn't model). **Remaining sub-increments (smaller): pipeline wiring for the per-type NR source (the METHOD already flows e2e via W-2 stateCodes); the part-year path's as-if-resident income-% method (it already sources per-type); a `CapitalGainTransaction.propertyStateSitus` field to auto-route real-estate gains; more method-(a)-verified states (each needs per-form verification — VA/AL/HI/IL/MA/MS/WV are method-b and must NOT be added).** Also: configurable retirement/SS exclusions advanced (CT SS, 2026-06-05g).
