...
### Planning engine
- [~] `[ENH]` **Model the credit/election mechanics that block what-if wiring** — **§23 adoption + §36B ACA PTC DONE 2026-06-06b; §41 R&D DONE 2026-06-06e.** §23 is a real engine credit (`calculateAdoptionCredit`) with **G1.65** engine-verified; **G1.30** reads the engine's actual Form 8962 reconciliation; **§41 R&D** is now a real engine credit (`calculateRdCredit`: ASC 14%/6%, §280C(c)(3) reduced election, §38(c) GBC liability limit with the TMT floor + §39 carryforward) with **G1.36** engine-verified when QRE is supplied. **Remaining:** §45S FMLA (G1.74), §51 WOTC (G1.75), §530 Coverdell (G1.59), §36B-full optimizer — each needs its `calculate*` modeled first. (NOTE: don't force-wire via the generic `credit` adjustment — it's refundable, would overstate for low-tax filers.)
