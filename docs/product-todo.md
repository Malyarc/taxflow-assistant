...
### Planning engine
- [~] `[ENH]` **Model the credit/election mechanics that block what-if wiring** — **§23 adoption + §36B ACA PTC DONE 2026-06-06b.** §23 is now a real engine credit (`calculateAdoptionCredit`: per-child cap, MAGI phase-out, OBBBA refundable split, §23(c) carryforward) and **G1.65 promoted** to engine-verified when a marker is present; **G1.30 promoted** to read the engine's actual Form 8962 reconciliation (`computed.premiumTaxCredit`) when a 1095-A is present (heuristic preserved otherwise). **Remaining:** §41 R&D (G1.36), §45S FMLA (G1.74), §51 WOTC (G1.75), §530 Coverdell (G1.59), §36B-full optimizer — each needs its `calculate*` modeled first. (NOTE: don't force-wire via the generic `credit` adjustment — it's refundable, would overstate for low-tax filers.)
- [~] `[ENH]` **H2-wire the remaining *engine-modelable* heuristics** (PLAN-Q2) — **§1244 (G1.40) DONE 2026-06-06b** (engine-verified current-year delta via recharacterize-loss → ordinary `deduction` mutation; what-if cross-checked vs an independent engine run). **§453 (G1.47) DONE 2026-06-06c** — H3 multi-year wired (baseline full-gain-year-0 vs scenario gain/N over 5 yrs; SAME total gain so no overstatement) via a new general `long_term_capital_gain` injection lever (Schedule D netting → AGI + preferential + NIIT + QBI cap); cross-checked vs an independent runMultiYearTrajectory pair. Exposed + fixed the flat-5% heuristic's two-way error ($20k→$4,353 for a $400k gain; $30k→$46k for $600k). **§1244/§453 pair COMPLETE. Remaining:** §163(d) investment-interest election (G1.93). (The ~44 *qualitative* G1.46–G1.96 detectors stay heuristic until their mechanic is modeled.)
