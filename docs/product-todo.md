...
- [x] `[ENH]` **Schedule C per-line P&L + depreciation** — ✅ **SE-BASE CORE DONE 2026-06-06f.** New `schedule_c_depreciation` adjustment (Form 4562) reduces the Schedule C net profit → SE-tax base + §199A QBI + earned income + §461(l), closing the documented gap (above-the-line §179/bonus reduced only AGI). Per-line P&L now complete: gross receipts (self_employment_income) − expenses (schedule_c_expenses) − depreciation = net SE. 8 hand-calc'd tests ($30k dep → SE tax −$4,238.87; §179 → SE tax −$0). **Remaining (assessed + DEFERRED 2026-06-06g — lower-value, "skip if tight"): an asset-level §179/bonus/MACRS calculator on the SE side.** The existing `calculateMacrsDepreciation` is REAL-PROPERTY ONLY (27.5/39-yr SL); a Sched C calculator needs personal-property MACRS classes (3/5/7/15-yr 200%DB, half-year/mid-quarter) + §179 with the business-income limitation + disallowed carryforward + bonus + a `scheduleCAssets` structured input on TaxReturnInputs (a migration-seam contract change) — a correctness-sensitive new subsystem, LOW reuse. A §179-only half-build would be worse than the CPA entering the Form 4562 figure (already SE-base-wired via `schedule_c_depreciation`). The per-EXPENSE-category breakdown stays out (cosmetic). PREP-B3 / Haven B#19.
