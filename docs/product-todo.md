...
- [x] `[ENH]` **Schedule C per-line P&L + depreciation** — ✅ **SE-BASE CORE DONE 2026-06-06f.** New `schedule_c_depreciation` adjustment (Form 4562) reduces the Schedule C net profit → SE-tax base + §199A QBI + earned income + §461(l), closing the documented gap (above-the-line §179/bonus reduced only AGI). Per-line P&L now complete: gross receipts (self_employment_income) − expenses (schedule_c_expenses) − depreciation = net SE. 8 hand-calc'd tests ($30k dep → SE tax −$4,238.87; §179 → SE tax −$0). **Remaining (documented, lower-value): per-EXPENSE-category breakdown (cosmetic) + an asset-level §179/bonus/MACRS calculator on the SE side (CPA supplies the Form 4562 figure).** PREP-B3 / Haven B#19.
...
- [~] `[ENH]` **Replace fixed heuristic splits with data** — **G1.17 DONE 2026-06-06f**: S-corp reasonable comp now uses a CPA-supplied benchmarked figure (`scorp_reasonable_comp`, an RC Reports/BLS OES result) instead of the hardcoded 40% (kept as a documented placeholder fallback; the engine doesn't invent a benchmark table). **Remaining:** per-state PTET regime audit for G1.2 — needs the AICPA state-by-state PTET data (which states, rates, caps, mechanics); a data task, NOT faked.
