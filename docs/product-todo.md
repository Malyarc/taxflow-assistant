...
- [~] `[ENH]` **Wire & ship the multi-year Roth optimizer** ⭐ — **v1 SHIPPED + DEPLOYED 2026-06-05e.** POST `/api/clients/:id/roth-optimizer` + `RothOptimizerCard` (Planning tab) + the existing pure solver, prod-verified (12% retiree fills the 12% bracket; 35% earner fills to $626,350; engine-exact costs). **Remaining (the long-term VALUE model) = the H3 multi-year hardening below** — RMD avoidance, IRMAA tiers, SS-taxability interaction. v1 discloses these as next-increment in its assumptions. (~6–8 wks for the H3 hardening that unlocks the value model)
- [ ] `[ENH]` **H3 multi-year engine hardening** (Roth value-model prereq) — `multiYearEngine.ts` currently freezes carryforwards + clamps every projection year to the latest bracket year. Add: carryforward depletion (NOL 80% §172 / cap-loss $3k §1212 / charitable 5-yr §170(d) / §163j), **RMD recognition at age 73** (§401(a)(9), Pub 590-B Table III), SS-taxability scaling (§86). Ordered plan + IRS cites in the 2026-06-05e scope investigation. (~6–8 wks)
...
- [~] `[ENH]` **Quick state wins** (PREP-Q3) — **VT dependent exemption + Yonkers PIT SHIPPED 2026-06-05e** (hand-calc'd, deployed); NJ retirement-exemption cap verified already-correct. **Remaining: WI std-ded high-AGI phase-out; CT pension/SS phase-out (currently over-taxes); IN-112 SS exclusion** — researched (thresholds + wiring plans in the 2026-06-05e scope investigation) but need final primary-source confirmation on the exact WI/CT thresholds before coding (won't ship a guessed phase-out rate over today's documented approximation). (hours–days each)
