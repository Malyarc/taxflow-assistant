...
- [~] `[ENH]` **H2-wire the remaining *engine-modelable* heuristics** (PLAN-Q2) — **§1244 (G1.40) DONE 2026-06-06b** (engine-verified current-year delta via recharacterize-loss → ordinary `deduction` mutation; what-if cross-checked vs an independent engine run). **§453 (G1.47) deliberately NOT single-year-wired** — its value is purely multi-year bracket-smoothing, so a single-year what-if would falsely book the deferred-year tax as a saving; honest wiring needs an installment-gain INPUT LEVER (recognize gain/N per year via runDetectorMultiYear; the engine has no general capital-gain adjustment to inject yet) — documented in the detector. **Remaining:** §163(d) investment-interest election (G1.93); §453 multi-year lever. (The ~44 *qualitative* G1.46–G1.96 detectors stay heuristic until their mechanic is modeled.)
