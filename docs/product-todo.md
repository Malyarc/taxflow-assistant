...
- [~] `[ENH]` **State "modifications" layer — next increments** (PREP-B1) — **configurable retirement/SS exclusions PARTLY advanced** (CT SS exclusion added 2026-06-05g; the per-state branch structure in `getStateRetirementExemption` is the data-driven seam). **Per-line NY IT-203 / CA 540NR sourcing is SCOPED but NOT shipped — genuinely multi-week.** The engine already has the foundation (C11 per-W-2/K-1/rental sourcing, the CA 540NR as-if-resident formula, days-prorated deductions, the former-state double-count fix). Remaining = full per-income-type sourcing (interest/div/gains/business/rental routed per-state with real-property-situs vs intangible-domicile) → needs a `CapitalGainTransaction.propertyStateSitus` schema field + per-type plumbing in `computePartYearAllocation` (~2-3 wks). Ordered plan + the 4 U.S.C. §114 retirement-preemption analysis in the 2026-06-05g scope investigation. The #1 correctness gap.
