...
- **Phase H batch shipped 2026-05-27 (6 of 12 items: H2 + H4 + H7 + H9 + H11 + H12) PLUS H1 partial (4 of 50+ strategies: G1.11 QCD + G1.12 appreciated stock + G1.13 Augusta Rule §280A(g) + G1.14 HSA maximization; catalog v1.2).** Pure primitive in `artifacts/api-server/src/lib/whatIfEngine.ts`. Threaded into the planning engine via `PlanningInputs.baselineInputs?: TaxReturnInputs`.
