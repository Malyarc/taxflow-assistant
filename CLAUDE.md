...
- **Test files (current set, 2,400+ assertions across 36 suites — ALL 10 K-list federal-engine gaps + ALL 4 state-engine gaps closed; C-batch 2026-05-26 shipped 6 new items (C1 coverage matrix + C4 Form 1040-X + C5 §1031 + C6 ESPP/ISO disqualifying + C7 §163(j)+§461(l) + C8 Form 4868) with 178 new hand-calc'd assertions across 5 new pure-engine test files; H2 what-if engine MVP shipped 2026-05-27 with 87 new hand-calc'd assertions in `tax-engine-whatif-tests.ts`; Phase G4 multi-year shipped with 70 new hand-calc assertions; Phase G5 Pro-tier gate shipped with 21 new dual-state integration assertions; Phase E shipped ALL 14 items with 235 hand-calc assertions total; deep-audit suite still 210 assertions, accuracy-audit 97 assertions; 0 documented federal/state gaps, 5 sub-gaps tracked (4 C-batch + 1 H2 partial wiring)):**
  | File | Needs API |
  |---|---|
  ...
  | `tax-engine-whatif-tests.ts` | no (H2 — 87 hand-calc'd assertions for `whatIfEngine.ts`: applyWhatIfMutations (set/add/remove adjustment, set client field, mutation order, purity, fresh-array semantics, disabled-adjustment preservation), runWhatIfScenario + runWhatIfScenarios (baseline-sharing, scenarioId pass-through, refund-sign convention), computeWhatIfDelta (null itemized handling, combined aggregates), CA state-tax delta non-zero. **Includes D1+D2 detector wire-up assertions** verifying G1.1 SEP-IRA attaches `whatIfDelta` when `baselineInputs` supplied and omits it otherwise (heuristic-only path preserved for /planning-hit-list).) |
...
- **H2 what-if engine shipped 2026-05-27 (MVP).** Pure primitive in `artifacts/api-server/src/lib/whatIfEngine.ts`. Takes a baseline `TaxReturnInputs` + list of `WhatIfMutation` (set/add/remove adjustment, set client field), applies mutations to a copy (no input mutation), re-runs `computeTaxReturnPure`, and returns a `WhatIfResult { baseline, scenario, delta }` where `delta.combinedTaxDelta` (federal + state liability) is the headline planning number — negative means the scenario reduces tax. Exposed via `POST /clients/{id}/what-if` endpoint (Pro-tier gated). Threaded into the planning engine: `PlanningInputs.baselineInputs?: TaxReturnInputs`; when present, `detectSepIra` (G1.1) attaches an engine-verified `whatIfDelta` to its `OpportunityHit`. Frontend Planning tab shows the verified delta in place of the heuristic estSavings and renders the per-field breakdown panel. **Sub-gap:** G1.4 Roth / G1.6 NIIT / G1.9 TLH / G1.10 FTC still emit heuristic estSavings only — each needs a strategy-specific mutation model (~3-5 days total). G1.3 / G1.7 / G1.8 are multi-year strategies — defer to H3.
