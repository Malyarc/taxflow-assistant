...
- **Test files (current set, 2,400+ assertions across 36 suites — ALL 10 K-list federal-engine gaps + ALL 4 state-engine gaps closed; C-batch 2026-05-26 shipped 6 new items (C1 coverage matrix + C4 Form 1040-X + C5 §1031 + C6 ESPP/ISO disqualifying + C7 §163(j)+§461(l) + C8 Form 4868) with 178 new hand-calc'd assertions across 5 new pure-engine test files; Phase H batch shipped 2026-05-27 (H2 expansion + H4 + H7 + H9 + H11 + H12) with 124 hand-calc'd assertions in `tax-engine-whatif-tests.ts` (Cases 1-20 + D1-D9); Phase G4 multi-year shipped with 70 new hand-calc assertions; Phase G5 Pro-tier gate shipped with 21 new dual-state integration assertions; Phase E shipped ALL 14 items with 235 hand-calc assertions total; deep-audit suite still 210 assertions, accuracy-audit 97 assertions; 0 documented federal/state gaps, 4 C-batch sub-gaps tracked, Phase H complete except H1/H3/H5/H6/H8/H10 (multi-week scope each)):**
  | File | Needs API |
  |---|---|
  ...
  | `tax-engine-whatif-tests.ts` | no (Phase H — 124 hand-calc'd assertions for `whatIfEngine.ts` + detector wire-ups: applyWhatIfMutations (set/add/remove adjustment, set client field, mutation order, purity, fresh-array semantics, disabled-adjustment preservation), runWhatIfScenario + runWhatIfScenarios (baseline-sharing, scenarioId pass-through, refund-sign convention), computeWhatIfDelta (null itemized handling, combined aggregates), CA state-tax delta non-zero. **D1-D9 detector wire-up + cross-strategy assertions:** D1/D2 G1.1 SEP with vs without baselineInputs; D3 G1.6 NIIT joint NIIT+ordinary savings; D4 G1.9 TLH bracket-aware delta (offsets LTCG at 15%, not ordinary 22%); D5 G1.10 FTC unlock via set_adjustment (uses combinedRefundDelta since credits don't reduce federalTaxLiability); D6 G1.4 Roth with `semantics="cost"`; D7 hit-list backward compat; D8 H7 cross-strategy stacked savings + interactionEffect math; D9 H7 omitted when 0 stackable.) |
...
- **Phase H batch shipped 2026-05-27 (6 of 12 items: H2 + H4 + H7 + H9 + H11 + H12).** Pure primitive in `artifacts/api-server/src/lib/whatIfEngine.ts`. Threaded into the planning engine via `PlanningInputs.baselineInputs?: TaxReturnInputs`.
  ...
  - Sub-gap: H1 / H3 / H5 / H6 / H8 / H10 are multi-week items pending their own sessions.
