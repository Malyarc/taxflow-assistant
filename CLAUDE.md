...
- **OBBBA core-engine items (2026-06-02, NOT applied — planning refresh handled the catalog side only):** the federal **SALT cap is still $10k (TCJA)** in `taxCalculator.ts` — the OBBBA $40k cap + >$500k-MAGI phase-down (§164(b)(7)) is NOT applied to the federal itemized total (the PTET planning detector works around it via its own `obbbaSaltCap()` off `saltUncapped`). Core **§199A SSTB thresholds** may still be TY2024-indexed and the OBBBA **$400 minimum QBI deduction** isn't applied in `calculateQbi`. **No native TY2026 support** — `resolveTaxYear` clamps TY2026→TY2025, so TY2026 returns use 2025 brackets/std-ded ($16,100/$32,200 not added); planning TY2026 dollar values are forward-staged (reached only when the planning layer is handed `taxYear=2026`). The 4 new OBBBA deductions (tips/overtime/car-loan/senior) are **planning-only** — not modeled as real adjustments in `computeTaxReturnPure`; the tips/overtime/car-loan markers also need API-enum + ClientForm UI to be CPA-enterable in prod (senior fires on age now). **Resolved:** TY2025 std-ded is now OBBBA-correct ($15,750/$31,500/$23,625).
...
- **2026-05-29 DEEP AUDIT #2 — 22 bugs fixed on branch `deep-audit-2026-05-29`** (two adversarially-verified multi-agent passes; 2 findings refuted + several downgraded; all fixes hand-calc'd + IRS/state-cited; 37 no-API suites / 2,943 assertions green; UI verified live):
  ...
  - **Still open (see `.claude/handoff.md`):** auth + multi-tenancy (D15, POSTPONED to Haven fusion); N×M planning query storm (DB-02/03); prod migration cutover (BLOCKED by a drizzle-kit `meta/_journal.json` snapshot-path bug); STL-05 (MD EITC two-component); god-file refactor; FE-03/04 frontend robustness; H2-wire heuristic planning detectors (≈G1.67–G1.96).
